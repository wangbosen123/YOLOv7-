...
class metrics():
    ...
    def plot_boxx(self, image, bounding_box, gt=True):
        ...
        if gt:
            ...
            label_name = 'Pred'
            color = [0, 0, 255]
        cv2.putText(image, label_name, org=(int(x1_1), int(y1_1)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=color, thickness=tf, lineType=cv2.LINE_AA)
    ...
    def based_on_slices_space(self):
        ...
        sn.heatmap(cm, xticklabels=["Tumor", "non-tumor"], yticklabels=["Tumor", "non-tumor"], annot=True, cmap='Blues', fmt=".0f")
        plt.title('confusion matrix')
        plt.xlabel('True label')
        plt.ylabel('Pred label')
        plt.savefig(f'result/cross5_validation_data_based_on_slices_confusion_matrix')
        plt.close()
        return TP, TN, FP, FN


    def based_on_boundingbox_space(self, iou_threshold, iterate=True):
        val_image_path = os.listdir(self.val_image_path)
        val_image_path = sorted(val_image_path, key=self.natural_sort_key)
    ...
    def validation_data_tumor_distribution(self, consecutive):
        TP, FN, TN, FP, TP_case, FN_case, FP_case = self.based_on_tumor_space(iou_threshold=0.001, conse=consecutive, lower_limit=13, upper_limit=65)
        case, tp_tumor_images, fn_tumor_images = [], [0], []
        case.append(int(os.listdir(self.val_gt_path)[0][5:8]))
        for filename in os.listdir(self.val_gt_path):
            if int(filename[5:8]) not in case:
                if int(filename[5:8]) in TP_case:
                    tp_tumor_images.append(0)
                else:
                    fn_tumor_images.append(0)
                case.append(int(filename[5: 8]))
        ...
        tp_tumor_images = np.array(tp_tumor_images) * 5
        fn_tumor_images = np.array(fn_tumor_images) * 5
        print(TP_case)
        print(FN_case)
        print(FP_case)

        plt.hist(tp_tumor_images, bins=20, color='blue', alpha=0.7, label='True Positive')
        plt.hist(fn_tumor_images, bins=20, color='red', alpha=0.7, label='False Negative')
        plt.title('Validation Datasets(Relation between tumor size and patient)')
        plt.xlabel('Size of Tumor(mm)')
        plt.ylabel('Number of Case')
        plt.savefig(f'result/cross4_validation_data_distribution')
        plt.close()


def iou_x_metrics_relation(iterate):
    for cross_num in range(1, 6):
        print(cross_num)
        total_tp, total_fn, total_tn, total_fp, total_iou = [], [], [], [], []
        detection_metrics = metrics(val_image_path=f'D:/yolov7/inference/cross{cross_num}/images/val/', val_gt_path=f'D:/yolov7/inference/cross{cross_num}/labels/val', val_pred_path=f'D:/yolov7/runs/detect/cross{cross_num}/labels/')
        for num, thre in enumerate([float(x/1000) for x in range(1001, 0, -100)]):
            print(f'iou is {thre}')
            TP, FN, TN, FP = detection_metrics.based_on_boundingbox_space(iou_threshold=thre, iterate=iterate)
            total_tp.append(TP)
            total_fn.append(FN)
            total_tn.append(TN)
            total_fp.append(FP)
            total_iou.append(thre)
        plt.plot(total_iou, total_tp, marker='o')
        plt.plot(total_iou, total_tn, marker='o')
        plt.plot(total_iou, total_fp, marker='o')
        plt.plot(total_iou, total_fn, marker='o')
        total_iou = [float(val) for val in total_iou]
        plt.grid(True)
        plt.legend(['TP', 'TN', 'FP', 'FN'], loc='upper right')
        plt.title('IoU and TP,TN,FP,FN relation (bounding box)')
        plt.xticks(total_iou)
        plt.tick_params(axis='x', which='both', bottom=True, top=False, labelbottom=True, width=10)
        plt.xlabel('IoU')
        plt.ylabel('number')
        plt.savefig(f'result/IoU and metrics relation cross{cross_num} iterate{iterate}')
        plt.close()
...
if __name__ == '__main__':
    ...
    # detection_metrics.validation_data_tumor_distribution(consecutive=3)
    iou_x_metrics_relation(iterate=True)
    iou_x_metrics_relation(iterate=False)
